...
export function YesterdayPage({ onUnauthorized, showToast }: YesterdayPageProps) {
  ...
  if (loading) return <div className="loading">正在加载昨日复盘...</div>;
    ...
    <>
      <header className="page-header">
        <div>
          ...
          <h1>昨日复盘</h1>
          <p className="muted">昨日是完整日；今天只是截至当前的参照，不做涨跌判断。</p>
        </div>
      </header>
      ...
      <section className="panel">
        ...
        <div className="trend-grid">
          <CompareMetric label="喂养" yesterday={yesterday.summary.feed_breast_count + yesterday.summary.feed_bottle_count} today={today.summary.feed_breast_count + today.summary.feed_bottle_count} unit="次" />
          <CompareMetric label="母乳" yesterday={yesterday.summary.breast_minutes_total} today={today.summary.breast_minutes_total} unit="分钟" duration />
          <CompareMetric label="奶瓶" yesterday={yesterday.summary.bottle_ml_total} today={today.summary.bottle_ml_total} unit="ml" />
          <CompareMetric label="小便" yesterday={yesterday.summary.pee_count} today={today.summary.pee_count} unit="次" />
          <CompareMetric label="大便" yesterday={yesterday.summary.poop_count} today={today.summary.poop_count} unit="次" />
          <CompareMetric label="睡眠" yesterday={yesterday.summary.sleep_minutes_total} today={today.summary.sleep_minutes_total} unit="分钟" duration />
        </div>
        <p className="notice">今天的数据还没完整，不用于判断变好或变差。</p>
      </section>
...
function CompareMetric({ label, yesterday, today, unit, duration = false }: { label: string; yesterday: number; today: number; unit: string; duration?: boolean }) {
  return (
    <article>
      <span>{label}</span>
      <strong>{duration ? formatDuration(yesterday) : `${formatNumber(yesterday)} ${unit}`}</strong>
      <small>今天截至当前：{duration ? formatDuration(today) : `${formatNumber(today)} ${unit}`}</small>
    </article>
  );
}
